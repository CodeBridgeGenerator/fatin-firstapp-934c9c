...
const ProductsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,description: _entity?.description,customer: _entity?.customer?._id,customer: _entity?.customer,
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                ...
                <Dropdown id="customer" value={_entity?.customer?._id} optionLabel="name" optionValue="value" options={~options~} onChange={(e) => setValByKey("customer", {_id : e.value})}  />
            </span>
            <small className="p-error">
